...
type LetterSpacingThemeKeys = keyof typeof theme.letterSparcings
...
export type LetterSacing = LetterSpacingThemeKeys|(String&{})
export type LineHeight = LineHeightThemeKeys|(String&{})

const BREAKPOINSTS:{[key:string]:string} = {
    sm:"640px",
    md:"768px",
    lg:"1024px",
    xl:"1280px",
    ...
    if(isResponsivePropType(prop)) {
        ...
        for(const responsiveKey in prop) {
            if(responsiveKey==="base") {
                ...
                const breakpoint = BREAKPOINSTS[responsiveKey]
...
function isLetterSpacingThemeKeys(prop:any,theme:AppTheme):prop is LetterSpacingThemeKeys {
    return Object.keys(theme.letterSparcings).filter((key)=>key==prop).length>0
}
